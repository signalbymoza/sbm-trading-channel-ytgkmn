...
export default function InstructionsTermsScreen() {
  const router = useRouter();
  const [termsAccepted, setTermsAccepted] = useState(false);

  console.log('InstructionsTermsScreen: Displaying instructions service terms and conditions');
  ...
  const handleContinue = () => {
    ...
    console.log('User accepted instructions service terms, navigating to registration');
    router.push('/registration?channel=education&program=instructions_service');
  };
  ...
  const note1En = 'Instructions and tips service is for SBM channel subscribers only.';
  const note1Ar = 'خدمة النصائح والتعليمات لمشتركي قناة SBM فقط.';

  const note2En = 'No refunds after payment.';
  const note2Ar = 'لا يوجد استرداد للمبلغ بعد الدفع.';
    ...
    <View style={styles.container}>
      ...
      >
        <View style={styles.header}>
          ...
          />
          <Text style={styles.title}>Instructions Service</Text>
          <Text style={styles.titleAr}>خدمة التعليمات</Text>
